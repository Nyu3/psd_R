...
## Excel reader for sevral sheets == (2020-10-12) ========================
getExcel <- function(...) {
    ## You cannot add sheet names as data type directly due to non-perfect reliability of them
    dL <- getData.(type = 'xls|xlsx|粒度調整', sheet_bind = F) %>% map(psdLab.)
    ...
    d <- list(assign_type(dL[[assign_num[1]]], 'ideal'), assign_type(dL[[assign_num[2]]], 'master'),
              assign_type(dL[[assign_num[3]]], 'A'), assign_type(dL[[assign_num[4]]], 'B'), assign_type(dL[[assign_num[5]]], 'C'),
              assign_type(dL[[assign_num[6]]], 'test')) %>% bind_rows()
