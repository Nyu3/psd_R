...
tunePSD <- function(...) {
    ...
    if (str_detect(d$sheet, 'test') %>% any()) {  # test info: tag << remark (ex. A12.3% : B87.7%)
        ...
        ## if no comment in the remark as NA, replace'em tothe tag name
        nm[str_detect(names(nm), 'test')] <- map2_chr(lege_remark_test, lege_tag_test, ~ replace_na(.x, replace = .y))
    }
    ...
    for (i in seq_along(pa)) for (j in seq_along(pb)) for (k in seq_along(pc)) {
        ...
        calc <- tibble(
            ...
            備考 = str_c(
                formattable::percent(fit$ratio[1], 2), ':',
                formattable::percent(fit$ratio[2], 2),
        ...
        if (length(pa) *length(pb) *length(pc) != 1) {
            if (i * j * k == 1) {
                ...
                pass_time <- if (time_length(time_obj, unit = 'sec') < 60) {
